...
{
	SearchingPlayerNum = 0;
	LastSessionSearch = MakeShareable(new FOnlineSessionSearch(DefaultSessionSearch));
	SessionInterface->FindSessions(SearchingPlayerNum, LastSessionSearch.ToSharedRef());
}
...
void UMultiplayerSubsystem::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	TArray<FName> SearchParamKeys;
	TArray<FOnlineSessionSearchParam> SearchParamValues;
	LastSessionSearch->QuerySettings.SearchParams.GenerateKeyArray(SearchParamKeys);
	LastSessionSearch->QuerySettings.SearchParams.GenerateValueArray(SearchParamValues);
	
	for (int i = 0; i < SearchParamKeys.Num(); ++i)
	{
		FName SearchParamKey = SearchParamKeys[i];
		//FOnlineSessionSearchParam SearchParamValue = SearchParamValues[i];
		UE_LOG(LogMultiplayerSubsystem, Display, TEXT("Search param key: %s"), *SearchParamKey.ToString());
	}
